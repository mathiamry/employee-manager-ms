...
export class EmployeeService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/employees', 'employeemanagerms');
  ...
  protected convertDateFromClient(employee: IEmployee): IEmployee {
    return Object.assign({}, employee, {
      dateOfHiring: employee.dateOfHiring?.isValid() ? employee.dateOfHiring.toJSON() : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dateOfHiring = res.body.dateOfHiring ? dayjs(res.body.dateOfHiring) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((employee: IEmployee) => {
        employee.dateOfHiring = employee.dateOfHiring ? dayjs(employee.dateOfHiring) : undefined;
      });
    }
    return res;
  }
